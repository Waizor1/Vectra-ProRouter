...
**Главный вывод: узкое место — не место на диске и не сеть, а RAM.**
15 из 22 не проходят порог 64 MB (у большинства 41-62 MB, и своп xray отнимет ещё ~10).
⇒ Волну начинать с ребута, иначе апдейт app будет падать без объяснений.
...
>
> **Не редкость: 2 случая из 4.** Повторилось на `avfilicity` (2026-08-06) — но там
> stage3 восстановил бинарь в том же автоматическом проходе, и простоя НЕ БЫЛО.
>
> Воспроизводится не везде. Наблюдения:
>
> | роутер | xray-core ДО апдейта | исход |
> |---|---|---|
> | artem-lutfulin | 26.6.1 | снесён |
> | avfilicity | 26.6.1 | снесён |
> | sairoutermsk | 26.4.25-r1 | уцелел |
> | VagrandRouter | 26.3.27-r1 | уцелел |
>
> Гипотеза (2 против 2, НЕ доказано): сносит там, где xray-core приехал с апстрим-фида
> (26.6.x), и не трогает пришедшие из Vectra-бутстрапа (26.3/26.4).
> **Опровергнуто и повторно не проверять:** дело НЕ в `Auto-Installed` — у всех
> выживших `Status: install user installed`, поля `Auto-Installed` в `/usr/lib/opkg/status`
> нет вовсе.
>
> Пока механизм не установлен — **исходить из худшего на каждом роутере**.
>
