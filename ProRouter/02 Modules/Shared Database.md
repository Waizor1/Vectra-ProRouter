...
- Добавлен локальный production-like upgrade harness: `apps/web/scripts/verify-db-upgrade-path.mjs` и wrapper `scripts/Test-VectraDbUpgradePath.ps1` проверяют clean bootstrap, upgrade поверх seeded old-state схемы, запуск sanitation pass и отсутствие повторного raw snapshot drift.
...
- Проверка production-like upgrade path остаётся недоказанной до успешного запуска `scripts/Test-VectraDbUpgradePath.ps1` при поднятом Docker daemon.
...
- Поднять Docker daemon и выполнить `scripts/Test-VectraDbUpgradePath.ps1`; затем решить, должен ли harness стать CI/pre-deploy gate.
