...
## Итог раскатки: 24 из 24 онлайн-роутеров (2026-08-07), парк закрыт

Весь достижимый парк на `app 26.7.16-r1` + `xray 26.7.28`, `Configuration OK.`
Вне охвата семь: `hh` (правило оператора), `sergeyavito` (direct), `totchto-filiciy`
(out_of_sync) и четыре офлайновых.

**`kirill-msk`: обновление ВЫЛЕЧИЛО его сломанный YouTube.** До — `generate_204` = 000
три раза и `i.ytimg.com` = 000, ПРИ ЭТОМ нода слота и WorldProxy тестировались на 204.
После — 204 пять раз подряд, нода 204@0.11 с, la 0.26 против 7.6-14.55 во время прошлого
инцидента с CPU. Причина была в локальном стеке, а не в узле.
⇒ **Диагностический признак: живая нода при мёртвых пробах — смотреть на версии стека,
а не менять ноду.**
Цена — **три простоя у клиентов** (`artem-lutfulin`, `zhenya13911`, `Vasily_Filicity`),
все три из-за драйвера, а не из-за роутеров. Уроки последнего этапа:

**`wget` на части роутеров не работает вовсе.** На `aleksandr-grigorievsky` он падал с
`Failed to send request: Operation not permitted`, тогда как `curl` тянул тот же файл за
2.7 с. Все загрузки переведены на curl, wget оставлен запасным.

**Проверку достижимости я делал через код возврата `wget`** — ту самую ловушку №4, которую
сам же и записал в этот файл. Из-за неё два ИСПРАВНЫХ роутера были ложно объявлены
заблокированными. Мерить только `curl -w %{http_code}` и с ретраем: доступ к github с
одного из роутеров оказался перемежающимся (то 200 и 19 МБ за 2.7 с, то 000).

**Джобы доставляются НЕ всем одинаково быстро.** У `aleksandr-grigorievsky` наблюдалась
доставка через 8 мин 19 с после постановки (сама команда потом отработала за 11 с), у
`denisvitalievichmain` — до 12 минут. Бюджет ожидания преф-лайта поднят до 900 с: ждать
дольше дешевле, чем ошибочно списать исправный роутер.

**Лекарство не должно быть опаснее болезни.** Мой же ретрай с `--force-login` из
нескольких потоков задолбил sshd на VPS (`Connection closed by ... port 22`), панель
отвалилась целиком — и вместе с ней аварийная сеть, которая ходит через панель. Это и
стоило простоя `Vasily_Filicity`. Логин теперь сериализован и не чаще раза в 90 с.

**Генерируемый шелл надо проверять `sh -n`.** Пропущенная точка с запятой перед `}`
уронила стадию на роутере; тесты на подстроки этого не ловили. Проверка синтаксиса всех
шести вариантов генерируемых скриптов добавлена в самотесты.

## Ручная установка app ОБЯЗАНА рестартовать сервис (2026-08-07)

`totchto-filiciy` лёг после успешной ручной распаковки app: файлы установлены, версия
верная, `initd=yes`, гео на месте — а весь проксируемый трафик в 000. Причина в том, что
панельный лейн перезапускает PassWall сам, а ручной путь этого не делал:
`/tmp/etc/passwall2/acl/default/global.json` не пересоздавался, и xray продолжал жить со
старым окружением. Лечится одним `/etc/init.d/passwall2 restart` — теперь он вшит в конец
ручного пути вместе с ожиданием подъёма процесса и проверкой, что конфиг появился.

**Попутно про `out_of_sync`.** У роутера накопилось шесть ночных ревизий `import_review`
подряд — подписка каждую ночь создаёт новую, и пока её не одобрить, апдейты заблокированы.
Разблокируется `fleet approve-import <селектор> -- --revision-id <id>`, но **сначала
смотреть, что именно одобряешь**: одобрение делает текущий живой конфиг эталоном. Проверять
`default_node=_direct`, привязки слотов к живым нодам и дата-план — и только потом жать.

**И про транзиенты установки зависимостей:** первый прогон отчитался, что `lyaml`,
`libyaml` и `coreutils-timeout` не встали, а ручной `opkg install` следом поставил все три
без единой правки. Гард при этом отработал верно — остановился до апдейта app.
