...
export type Ax3000tManagedPackagePlan = {
  ...
  controllerOnlyFallback: boolean;
...
export function planAx3000tManagedPackageOperations(args: {
  ...
  if (classification === "fresh install") {
    ...
    for (const packageName of AX3000T_REQUIRED_MIRRORED_INSTALL_ORDER) {
      ...
      if (simulatedOverlayFreeBytes < target.installedSizeBytes) {
        return {
          classification,
          controllerOnlyFallback: true,
          passwallAppRemovalRequired: false,
          requiredStageBytes: 0,
          steps,
          storageCheck: {
            ok: false,
            reason: "overlay",
            ...
            requiredBytes: target.installedSizeBytes,
            reclaimBytes: 0,
            unexpectedDependents: [],
            message: buildManagedStorageMessage(
              "overlay",
              packageName,
              simulatedOverlayFreeBytes,
              target.installedSizeBytes,
              0,
              [],
            ),
          },
      ...
      simulatedOverlayFreeBytes -= target.installedSizeBytes;
    }
  }
...
const AX3000T_SHUNT_DEFAULT_REBIND_TARGET = AX3000T_SHUNT_REBIND_TARGETS[0];
...
export function buildAx3000tShuntRebindScript(args?: {
  ...
  const applySlotCalls = [
    ...AX3000T_SHUNT_REBIND_TARGETS.map(
      ({ slot, remark }) => `apply_slot '${slot}' '${remark}'`,
    ),
    `apply_slot 'default_node' '${AX3000T_SHUNT_DEFAULT_REBIND_TARGET.remark}'`,
  ];
...
export function buildAx3000tBootstrapScript(args: {
  ...
  return [
    ...
    `OPENWRT_MIRROR_URL='${routerApiBase}/openwrt-cache'`,
    "OPENWRT_FEED_PROXY_ACTIVE='0'",
    "",
    "log() {",
    "  printf '%s\\n' \"$*\"",
    "}",
    ...
    "fi",
    "",
    "ensure_opkg_architecture() {",
    "  arch_name=\"$1\"",
    "  arch_conf='/etc/opkg/arch.conf'",
    "  mkdir -p /etc/opkg",
    "  [ -f \"$arch_conf\" ] || : > \"$arch_conf\"",
    "  ensure_arch_line() {",
    "    name=\"$1\"",
    "    priority=\"$2\"",
    "    if ! grep -Eq \"^[[:space:]]*arch[[:space:]]+${name}([[:space:]]|$)\" \"$arch_conf\" 2>/dev/null; then",
    "      printf 'arch %s %s\\n' \"$name\" \"$priority\" >> \"$arch_conf\"",
    "      log \"opkg arch.conf: добавил архитектуру $name\"",
    "    fi",
    "  }",
    "  ensure_arch_line all 1",
    "  ensure_arch_line noarch 1",
    "  ensure_arch_line \"$arch_name\" 100",
    "  if ! opkg print-architecture 2>/dev/null | awk '{ if ($1 == \"arch\") print $2; else print $1 }' | grep -qx \"$arch_name\"; then",
    "    log \"opkg всё ещё не видит архитектуру $arch_name; зависимости вроде libc могут не разрешиться.\"",
    "  fi",
    "}",
    "",
    ...
    "restore_openwrt_feeds() {",
    "  if [ \"$OPENWRT_FEED_PROXY_ACTIVE\" = '1' ] && [ -f \"$BACKUP_ROOT/distfeeds.conf.orig\" ]; then",
    "    cp \"$BACKUP_ROOT/distfeeds.conf.orig\" /etc/opkg/distfeeds.conf 2>/dev/null || true",
    "    OPENWRT_FEED_PROXY_ACTIVE='0'",
    "    log 'opkg distfeeds возвращены к штатным OpenWrt-зеркалам.'",
    "  fi",
    "}",
    "cleanup() {",
    "  restore_openwrt_feeds",
    ...
    "",
    "if ! run_opkg update; then",
    "  log 'opkg update завершился с предупреждениями; продолжаю, потому что package-specific preflight проверит нужные пакеты отдельно.'",
    "fi",
    "",
    ...
    "}",
    "",
    "openwrt_base_prereq_present() {",
    "  pkg=\"$1\"",
    "  case \"$pkg\" in",
    "    libc)",
    "      [ -e /lib/libc.so ] || ls /lib/ld-musl-*.so.1 >/dev/null 2>&1",
    "      ;;",
    "    *)",
    "      return 1",
    "      ;;",
    ...
    "  [ \"$storage_budget\" -gt 0 ] || {",
    "    log \"Предупреждение: не удалось определить storage budget для OpenWrt пакета $pkg (opkg info без Installed-Size/Size); пропускаю storage-aware проверку для него.\"",
    "    return 0",
    "  }",
    "  [ \"$download_size\" -gt 0 ] || {",
    "    log \"Предупреждение: не удалось определить Size для OpenWrt пакета $pkg; пропускаю storage-aware проверку для него.\"",
    "    return 0",
    "  }",
    "}",
    "",
    ...
    "}",
    "",
    "try_openwrt_feed_proxy() {",
    "  [ -n \"$OPENWRT_MIRROR_URL\" ] || return 1",
    "  [ \"$OPENWRT_FEED_PROXY_ACTIVE\" = '1' ] && return 1",
    "  [ -f /etc/opkg/distfeeds.conf ] || return 1",
    "  grep -q 'downloads.openwrt.org' /etc/opkg/distfeeds.conf || return 1",
    "  mkdir -p \"$BACKUP_ROOT\"",
    "  if [ ! -f \"$BACKUP_ROOT/distfeeds.conf.orig\" ]; then",
    "    cp /etc/opkg/distfeeds.conf \"$BACKUP_ROOT/distfeeds.conf.orig\" || return 1",
    "  fi",
    "  log \"OpenWrt feeds недоступны напрямую (частая причина — битый IPv6 до downloads.openwrt.org); переключаю opkg на кеширующий прокси Vectra: $OPENWRT_MIRROR_URL\"",
    "  sed -e \"s#https://downloads.openwrt.org#$OPENWRT_MIRROR_URL#g\" -e \"s#http://downloads.openwrt.org#$OPENWRT_MIRROR_URL#g\" \"$BACKUP_ROOT/distfeeds.conf.orig\" > /etc/opkg/distfeeds.conf || {",
    "    cp \"$BACKUP_ROOT/distfeeds.conf.orig\" /etc/opkg/distfeeds.conf 2>/dev/null || true",
    "    return 1",
    "  }",
    "  OPENWRT_FEED_PROXY_ACTIVE='1'",
    "  run_opkg update >/dev/null 2>&1 || true",
    "  return 0",
    "}",
    "",
    "ensure_openwrt_feeds_usable() {",
    "  sentinel_pkg='dnsmasq-full'",
    "  if package_installed \"$sentinel_pkg\"; then",
    "    return 0",
    "  fi",
    "  if package_available \"$sentinel_pkg\"; then",
    "    return 0",
    "  fi",
    "  if try_openwrt_feed_proxy && package_available \"$sentinel_pkg\"; then",
    "    log 'OpenWrt feeds доступны через кеширующий прокси Vectra; продолжаю полную установку (PassWall2/Xray + контроллер).'",
    "    return 0",
    "  fi",
    "  log \"OpenWrt feeds недоступны: $sentinel_pkg не виден в opkg lists после opkg update.\"",
    "  if [ \"$BOOTSTRAP_CLASSIFICATION\" = 'fresh install' ]; then",
    "    CONTROLLER_ONLY_BOOTSTRAP='1'",
    "    PASSWALL_APP_REMOVAL_REQUIRED='0'",
    "    log 'Свежая установка при недоступных OpenWrt feeds: dnsmasq-full и kmod-* сейчас не скачать с downloads.openwrt.org.'",
    "    log 'Продолжаю controller-only bootstrap: ставлю контроллер Vectra (его зависимости есть в базовом образе) без PassWall2/Xray.'",
    "    log 'PassWall2/Xray довключим из панели Vectra управляемой задачей после первого check-in, когда фиды снова доступны.'",
    "    return 0",
    "  fi",
    "  log 'Bootstrap не может продолжаться: dnsmasq-full и kmod-* надо ставить из downloads.openwrt.org, а opkg update не смог скачать metadata.'",
    "  log 'Проверьте сеть на роутере и перезапустите bootstrap:'",
    "  log '  - ip route show default'",
    "  log '  - cat /etc/resolv.conf'",
    "  log '  - ping -c 2 -W 5 downloads.openwrt.org'",
    "  log '  - nslookup downloads.openwrt.org'",
    "  log 'Если резолвер не отвечает, временно добавьте публичный DNS и повторите запуск:'",
    "  log \"  echo 'nameserver 1.1.1.1' >> /etc/resolv.conf\"",
    ...
    "}",
    "",
    "mark_controller_only_bootstrap() {",
    "  pkg=\"$1\"",
    "  available_bytes=\"$2\"",
    "  required_bytes=\"$3\"",
    "  reclaim_bytes=\"$4\"",
    "  CONTROLLER_ONLY_BOOTSTRAP='1'",
    "  PASSWALL_APP_REMOVAL_REQUIRED='0'",
    "  log \"Недостаточно места на /overlay для стартового PassWall2/Xray stack ($pkg): доступно ${available_bytes} B, требуется ${required_bytes} B.\"",
    "  if [ \"$reclaim_bytes\" -gt 0 ]; then",
    "    log \"Даже после reclaim $pkg доступно только $((available_bytes + reclaim_bytes)) B.\"",
    "  fi",
    "  log 'Продолжаю controller-only bootstrap: ставлю контроллер Vectra без PassWall2/Xray.'",
    "  log 'PassWall2/Xray обновим позже из панели Vectra через контроллер после первого check-in.'",
    "}",
    "",
    "is_passwall_bootstrap_package() {",
    "  case \" $REQUIRED_MANAGED_PACKAGES $REQUIRED_RUNTIME_PACKAGES \" in",
    "    *\" $1 \"*) return 0 ;;",
    "    *) return 1 ;;",
    "  esac",
    "}",
    ...
    "  required_bytes=\"$3\"",
    "  reclaim_bytes=\"$4\"",
    "  if [ \"$BOOTSTRAP_CLASSIFICATION\" = 'fresh install' ] && is_passwall_bootstrap_package \"$pkg\"; then",
    "    mark_controller_only_bootstrap \"$pkg\" \"$available_bytes\" \"$required_bytes\" \"$reclaim_bytes\"",
    "    return 0",
    "  fi",
    ...
    "  [ \"$required_bytes\" -gt 0 ] || {",
    "    log \"Предупреждение: не удалось определить storage budget для $pkg (opkg info без Installed-Size/Size); пропускаю overlay-симуляцию для него.\"",
    "    return 0",
    "  }",
    ...
    "}",
    "",
    "detect_fresh_passwall_overlay_shortage() {",
    "  [ \"$BOOTSTRAP_CLASSIFICATION\" = 'fresh install' ] || return 0",
    "  simulated_overlay_bytes=\"$CURRENT_OVERLAY_FREE_BYTES\"",
    "  for pkg in $REQUIRED_MANAGED_PACKAGES; do",
    "    target_bytes=\"$(managed_package_field \"$pkg\" installed_size_bytes)\"",
    "    [ \"$target_bytes\" -gt 0 ] || continue",
    "    if [ \"$simulated_overlay_bytes\" -lt \"$target_bytes\" ]; then",
    "      mark_controller_only_bootstrap \"$pkg\" \"$simulated_overlay_bytes\" \"$target_bytes\" '0'",
    "      return 0",
    "    fi",
    "    simulated_overlay_bytes=$((simulated_overlay_bytes - target_bytes))",
    "  done",
    "}",
    "",
    ...
    "  if [ \"$CONTROLLER_ONLY_BOOTSTRAP\" != '1' ] && [ \"$REUSE_EXISTING_PASSWALL_STACK\" != '1' ]; then",
    "    for pkg in $REQUIRED_MANAGED_PACKAGES; do",
    "      if ! managed_package_at_target \"$pkg\"; then",
    ...
    "      done",
    "    fi",
    "    if [ \"$blocking_stage_pkg\" = 'unknown' ] && [ \"$CONTROLLER_ONLY_BOOTSTRAP\" != '1' ]; then",
    "      for pkg in $REQUIRED_RUNTIME_PACKAGES; do",
    "        if runtime_package_needs_install \"$pkg\"; then",
    ...
    "  if [ \"$CONTROLLER_ONLY_BOOTSTRAP\" != '1' ] && runtime_package_needs_install dnsmasq-full; then",
    ...
    "    fi",
    "    simulate_overlay_step dnsmasq-full \"$dnsmasq_target_bytes\" \"$dnsmasq_reclaim_bytes\" || true",
    "  fi",
    "  if [ \"$CONTROLLER_ONLY_BOOTSTRAP\" != '1' ]; then",
    `    for pkg in ${requiredRuntimePackagesWithoutDnsmasq}; do`,
    "      [ \"$CONTROLLER_ONLY_BOOTSTRAP\" = '1' ] && break",
    "      if runtime_package_needs_install \"$pkg\"; then",
    "        target_bytes=\"$(feed_package_storage_budget_bytes \"$pkg\")\"",
    "        simulate_overlay_step \"$pkg\" \"$target_bytes\" '0' || true",
    "      fi",
    "    done",
    "  fi",
    "  if [ \"$CONTROLLER_ONLY_BOOTSTRAP\" = '1' ]; then",
    "    SIMULATED_OVERLAY_FREE_BYTES=\"$CURRENT_OVERLAY_FREE_BYTES\"",
    "  fi",
    "  if [ \"$CONTROLLER_ONLY_BOOTSTRAP\" != '1' ] && [ \"$PASSWALL_APP_REMOVAL_REQUIRED\" = '1' ] && package_installed luci-app-passwall2; then",
    ...
    "      fi",
    "      target_bytes=\"$(managed_package_field \"$pkg\" installed_size_bytes)\"",
    "      simulate_overlay_step \"$pkg\" \"$target_bytes\" \"$reclaim_bytes\" || true",
    "    done",
    "    if [ \"$CONTROLLER_ONLY_BOOTSTRAP\" != '1' ] && { [ \"$PASSWALL_APP_REMOVAL_REQUIRED\" = '1' ] || ! managed_package_at_target luci-app-passwall2; }; then",
    "      target_bytes=\"$(managed_package_field luci-app-passwall2 installed_size_bytes)\"",
    "      simulate_overlay_step luci-app-passwall2 \"$target_bytes\" '0' || true",
    "    fi",
    "  fi",
    "  for pkg in $CONTROLLER_PACKAGES; do",
    "    if controller_package_at_target \"$pkg\"; then",
    ...
    "  if [ \"$CONTROLLER_ONLY_BOOTSTRAP\" = '1' ]; then",
    "    log 'Controller-only lane: стартовый PassWall2/Xray stack не помещается, тяжёлые пакеты будут обновлены позже из панели.'",
    "  elif [ \"$REUSE_EXISTING_PASSWALL_STACK\" = '1' ]; then",
    ...
    "",
    "if [ \"$CONTROLLER_ONLY_BOOTSTRAP\" = '1' ]; then",
    "  log 'Controller-only bootstrap: пропускаю dnsmasq-full и PassWall2 runtime-пакеты в установщике.'",
    "else",
    ...
    "}",
    "",
    "# api.to_move() writes to get_app_path(component): the uci value",
    "# passwall2.@global_app[0].<component>_file, or com[<component>].default_path",
    "# (/usr/bin/<component>) when that value is empty. On low-RAM boards we point",
    "# the uci value at /usr/sbin/vectra-xray-wrapper, the GOMEMLIMIT/GOGC shim that",
    "# execs /usr/bin/xray. Running the built-in updater there overwrites the wrapper",
    "# with the raw binary, leaves the real /usr/bin/xray stale, and adds a second",
    "# full-size copy to the overlay -- while still reporting success. Allow the",
    "# built-in updater only when it would land on the package-owned path.",
    "passwall_component_updater_targets_managed_binary() {",
    "  component=\"$1\"",
    "  expected=\"/usr/bin/$component\"",
    "  key=\"$(printf '%s' \"$component\" | tr '-' '_')\"",
    "  configured=\"$(uci -q get \"passwall2.@global_app[0].${key}_file\" 2>/dev/null)\"",
    "  [ -n \"$configured\" ] || configured=\"$expected\"",
    "  if [ \"$configured\" != \"$expected\" ]; then",
    "    log \"[$component] passwall2 ${key}_file указывает на $configured, а не на $expected; встроенный App Update пропущен, чтобы api.to_move не затёр wrapper.\"",
    "    return 1",
    "  fi",
    "  if [ -f \"$configured\" ] && [ \"$(head -c 2 \"$configured\" 2>/dev/null)\" = '#!' ]; then",
    "    log \"[$component] $configured — скрипт-обёртка, а не бинарь; встроенный App Update пропущен.\"",
    "    return 1",
    "  fi",
    "  return 0",
    "}",
    ...
    `  apply_shunt_slot 'default_node' '${AX3000T_SHUNT_DEFAULT_REBIND_TARGET.remark}'`,
    "  if [ \"$changes_made\" = '1' ]; then",
    "    uci commit passwall2",
    "  fi",
    "}",
    ...
    "      }",
    "    fi",
    "    log \"[$pkg] устанавливаю $target_version из подписанного feed Vectra до запуска PassWall2...\"",
    "    run_opkg install \"$pkg\" || {",
    ...
    "}",
    "",
    "passwall_bootstrap_ready_to_start() {",
    "  selected_node=\"$(uci -q get passwall2.vectra_global.node || true)\"",
    "  case \"$selected_node\" in",
    "    ''|'_direct'|'_default'|'_blackhole')",
    "      log \"PassWall2 start guard: выбранный узел '$selected_node' не является сервером; оставляю PassWall2 выключенным.\"",
    "      return 1",
    "      ;;",
    "  esac",
    "  if ! node_exists \"$selected_node\"; then",
    "    log \"PassWall2 start guard: выбранный узел '$selected_node' не найден; оставляю PassWall2 выключенным.\"",
    "    return 1",
    "  fi",
    "  selected_protocol=\"$(uci -q get \"passwall2.$selected_node.protocol\" || true)\"",
    "  if [ \"$selected_protocol\" != '_shunt' ]; then",
    "    return 0",
    "  fi",
    "  default_node=\"$(uci -q get \"passwall2.$selected_node.default_node\" || true)\"",
    "  case \"$default_node\" in",
    "    ''|'_direct'|'_default'|'_blackhole')",
    "      log \"PassWall2 start guard: shunt Default/default_node='$default_node' не привязан к серверу; оставляю PassWall2 выключенным.\"",
    "      return 1",
    "      ;;",
    "  esac",
    "  if ! node_exists \"$default_node\"; then",
    "    log \"PassWall2 start guard: shunt Default/default_node '$default_node' не найден; оставляю PassWall2 выключенным.\"",
    "    return 1",
    "  fi",
    "  if [ \"$(uci -q get \"passwall2.$default_node.protocol\" || true)\" = '_shunt' ]; then",
    "    log \"PassWall2 start guard: shunt Default/default_node '$default_node' указывает на другой shunt; оставляю PassWall2 выключенным.\"",
    "    return 1",
    "  fi",
    "  return 0",
    "}",
    ...
    "  if ! passwall_bootstrap_ready_to_start; then",
    "    uci set passwall2.vectra_global.enabled='0' || true",
    ...
    "  start_passwall_after_bootstrap",
    "else",
    "  log 'Устанавливаю пакеты контроллера Vectra без запуска PassWall2...'",
    "  install_controller_packages",
    "  log 'Controller-only bootstrap: пропускаю установку/настройку PassWall2, baseline, подписки и запуск сервиса.'",
    "  log 'После первого check-in запусти обновление PassWall2/Xray из панели Vectra; контроллер выполнит это управляемой задачей.'",
    "fi",
    "",
    "uci batch <<'EOF'",
    ...
    "log 'Готово.'",
    "if [ \"$CONTROLLER_ONLY_BOOTSTRAP\" = '1' ]; then",
    "  log \"1. Контроллер подключён к $CONTROL_URL\"",
    "  log '2. PassWall2/Xray не менялись во время установки из-за малого /overlay; обновление делаем позже управляемо из панели Vectra.'",
    "  log '3. После первого check-in откройте веб-панель, примите импорт и запустите обновление PassWall2/Xray.'",
    "else",
