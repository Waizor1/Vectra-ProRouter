import type { RouterYoutubeReachability } from "@vectra/contracts";

type YoutubeReachabilityCheckLike = {
  label?: string | null;
...
type YoutubeReachabilityLike =
  | (Partial<RouterYoutubeReachability> & YoutubeReachabilityCheckLike)
  | null
  | undefined;

type YoutubeReachabilityStatus =
  | "unknown"
  | "reachable"
  | "partial"
  | "blocked";

export type YoutubeReachabilityCheckSummary = {
  label: string;
  reachable: boolean;
  checkedAt?: string | null;
  detail: string;
...
function getChecks(probe: YoutubeReachabilityLike) {
  if (!probe) {
    return [] as YoutubeReachabilityCheckLike[];
  }
...
export function getYoutubeReachabilityStatus(
  probe: YoutubeReachabilityLike,
): YoutubeReachabilityStatus {
  if (!probe) {
    return "unknown";
  }
...
export function formatYoutubeReachabilityLabel(probe: YoutubeReachabilityLike) {
  switch (getYoutubeReachabilityStatus(probe)) {
    case "reachable":
      return "доступна";
    case "partial":
...
function describeSingleCheck(check: YoutubeReachabilityCheckLike) {
...
export function describeYoutubeReachability(probe: YoutubeReachabilityLike) {
  ...
  if (status === "reachable") {
    return `Отвечают все ${totalCount} цели YouTube.`;
  }
  ...
  if (status === "partial") {
    const suffix =
      failingLabels.length > 0
        ...
        : "";
    return `Отвечают ${reachableCount} из ${totalCount} целей YouTube.${suffix}`;
  }

  return `Не отвечает ни одна из ${totalCount} целей YouTube.`;
}

export function getYoutubeReachabilityChecks(
  probe: YoutubeReachabilityLike,
): YoutubeReachabilityCheckSummary[] {
...
export function hasYoutubeReachabilityProblem(probe: YoutubeReachabilityLike) {
  const status = getYoutubeReachabilityStatus(probe);
