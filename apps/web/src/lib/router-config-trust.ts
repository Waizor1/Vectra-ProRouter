...
export function describeConfigTrustState(args: {
  ...
  if (requiresReimport) {
    return {
      badge: digestMismatch ? "нужна сверка" : "проверяем настройки",
      badgeClassName: "border-amber-400/30 bg-amber-500/12 text-amber-100",
      title: digestMismatch
        ? "Панель видит изменения вне интерфейса"
        : "Подробные настройки ещё обновляются",
      detail:
        "Панель уже видит связь, выбранную ноду и версии, но подробные разделы PassWall2 ещё подтягиваются. Обычно это фоновая сверка; вручную обновляйте данные только после правок через LuCI/SSH или при реальном конфликте.",
    };
  }
  ...
  if (mode === "authoritative") {
    return {
      ...
      detail:
        "Это нормальный рабочий режим: панель опирается на сохранённую базу и может с ней работать. Если настройки не меняли вне панели, ручная сверка не нужна.",
    };
  }
  ...
  return {
    ...
    detail:
      "Панель сравнила сохранённую базу с последним чтением настроек самого роутера. Это самый надёжный вариант для подробных разделов PassWall2.",
  };
}
